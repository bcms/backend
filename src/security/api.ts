...
export function createBcmsApiKeySecurity(): Module {
  return {
    ...
    initialize(moduleConfig) {
      const apiKeyRepo = useBcmsApiKeyRepository();
      const usedRequests: {
        [id: string]: {
          expAt: number;
        };
      ...
      apiKeySecurity = {
        async verify(request, skipAccess) {
          ...
          const key = await apiKeyRepo.findById(request.data.k);
          if (!key) {
        ...
        httpRequestToApiKeyRequest(request) {
          return {
            data: {
...
export function useBcmsApiKeySecurity(): BCMSApiKeySecurity {
...
export function createBcmsApiKeySecurityPreRequestHandler(): ControllerMethodPreRequestHandler<{
  ...
  return async ({ request, errorHandler }) => {
    try {
      return {
        apiKey: await apiKeySecurity.verify(
          apiKeySecurity.httpRequestToApiKeyRequest(request),
        ),
      };
    } catch (error) {
